...
'''
usage = '''
python3 python3 TEpipeline_script_v1.1.py -t Test_SGDP_BAMid.txt -p /kbod2/WGS_DATA/SGDP_bams_public -o /ybod2/jainy/
Project3/SGDP_outputdir -cov /home/jainy/software/goleft_0.2.0/goleft_linux64 -m /home/jainy/software/MELTv2.1.5 -ref 
/vbod2/jainy/SGDP/Ref-SGDP/UCSC_hg38.fa -pro 10 -mem 5 -rm /vbod2/jainy/SGDP/hg38_Repeatmasker/hg38.fa.out.sorted.bed 
-n 4 -sc 1 -An /ybod/jainy/bin/AnnotSV/AnnotSV_2.3 -pt /home/jainy/software/picard-2.18.23 -b /home/jainy/software/nc
bi-blast-2.7.1+/bin -l /ybod2/jainy/Project3/Human_TEs_rep.fasta -mu /home/jainy/software/muscle3.8.31/muscle3.8.31_i
86linux64
       '''
...
parser.add_argument("-p", "--path", help="location of bam files", type=str)
parser.add_argument("-r", "--regions", help="list of genomic coordinates for extracting reads")
parser.add_argument("-b", "--blastpro", help="path of blast program")
parser.add_argument("-q", "--qseq", help="seq for which homology to be tested")
parser.add_argument("-o", "--outputdir", help="path and name of the output dir")
parser.add_argument("-a", "--assembler", help="path of the spade assembler")
parser.add_argument("-c", "--cap3", help="path of the cap3 assembler")
parser.add_argument("-m", "--meltpath", help="path of the MELTtool")
parser.add_argument("-ref", "--ref_genome", help="path of the reference genome")
parser.add_argument("-cov", "--coverage", help="path of the coverage finder tool or coverage output can be provided "
                                               "(suffix should be *.coverage.txt)")
parser.add_argument("-pro", "--cpu", help="max no. of cpus available")
parser.add_argument("-mem", "--memmory", help="memmory available for each process")
parser.add_argument("-vhg", "--hg_ver", help="version of the human genome")
parser.add_argument("-rm", "--repmask", help="path to repeatmasker file")
parser.add_argument("-An", "--ASVpath", help = "path to AnnotSV")
parser.add_argument("-n","--totaln", help = " total number of individuals analysed")
parser.add_argument("-sc","--scoren", help = " min ANNOTSV score to be analysed")
parser.add_argument("-si","--segind", help = " individuals segregating")
parser.add_argument("-ni","--nonsegind", help = " individuals non segregating")
parser.add_argument("-mu","--muscle", help = "path of the MUSCLE aligner")
parser.add_argument("-l","--TElib",help = "path of the TElibrary")
parser.add_argument("-pt","--picardtool",help = "path of the picard tool")
...
# Todo: get list ofTEs to analyse, include a config file to include all the paths, cpus, mem
# Todo: write in the status file only the programs successfully completed use a parameter to check if sucessfully ran
...
if not args.hg_ver:
    args.hg_ver = "hg38"
if not args.scoren:
    args.scoren = 3 # 3 = VUS, 4=likely pathogenic, 5 pathogenic, 2 = likely benign 1= benign

if not args.nonsegind:
    args.nonsegind =0
if not args.segind:
    args.segind = "None"
...
#cutoffnumber = 9 #cut off number of clipped reads Needs to be determined from coverage
# global bwapath
bwapath = "~/software/bwa"

samtoolspath = "~/miniconda3/bin"
transpath = "/home/jainy/software/TranSurVeyor"
#fix_bampath = "/ybod2/jainy/Project3/hg19_1KGP_bams_mate"
bedtoolp = "~/software/bedtools2/bin/"
...
# outputfile = args.table+".merged"
# make a config file for the path of the programs, for now add in to commandline
####################################___FUNCTIONS___########################################
###########################################################################################
...
def parse_Transurveyor_out(outdir, coverage_dict):
    ...
    # Todo: may be filter out that do not have TE masking when dealing with big data to make file smaller
    # Todo: Merging can be done specifically for Transurveyor output since MELT has already dealt with it
    dictallbrkpt = {}
...
def intersectBed_RMdata(cord_dict,rdict):
    dfObj = pd.DataFrame.from_dict(cord_dict, orient='index', columns=["SVLine"])
    # convert dict to dataframe (chr, start, end, uniqueid,frequencyofoccurence)
    # sort the data in the ascending order and print the data in to file with filename bam name
    # intersect the two bedfiles
    # output the file

def parse_intersectBedoutput(bedoutputfile,dict):
    #parse the output file and add the info on the loci to the dict.
    print("bedoutputfile")
...
def call_TSD_to_identifybrkpt(anSVout1,coverage_dict, bampath,musclepath,TElibpath,outdir,cpus,blastpath,totalNumI):
    ...
    RMdata_dict = load_Repeatmasker(args.repmask)
                ...
                for read in samfile.fetch(chr, int(start), int(end)):
                    ...
                    if read.is_qcfail == True or read.is_duplicate == True:
                        ...
                        if read.is_proper_pair == True:
                            ...
                            '''#Todo:get the positions of mates of discordant reads
                            #check in the RMdata table to see the TE family it maps to
                            #Get the TEfamily and count occurance
                            # get the mates of discordant reads from all individuals, then wont be clear will it be present in that individual.
                            # planning to do that for each individual, in that case wont be needed to that step for the Transurveyor output. would have to modify that part (deactivate the find_corresponding_TE in parse surveyorout function ). For now let it be there.
                            #output the TEfamily and # of discordant reads
                            #If I implement this I can may be avoid similar step with Transurveryor when checking the other breakpoint for TE and modify that code
'''
                ...
                if not TSDcord == "None":
                    ...
                    if args.muscle:
                        ...
                        leftconsensus = align_consensus(TSDcord, LeftSoftclipseq, "left", args.outputdir, bam,
                                                        musclepath)
                        rightconsensus = align_consensus(TSDcord, RighSoftclipseq, "right", args.outputdir, bam,
                                                         musclepath)
    ...
    print_finalout(args.outputdir, "refinedtsd.txt", TSD_cord)
    #print(TSD_status,"TSDrecord-uniqueid dict")
    fileforASVrd2 = print_finalout_hori(args.outputdir, "filetorunAnSV_2.bed", gtdata, headerforcombgttb,totalNumI)
...
###########################################################################################
# open file and load the data into a dictionary
Bamid_dict = openfiletoload(args.table)
print(Bamid_dict)
# create a directory
check_dir_exists(args.outputdir)
status_checkdir = args.outputdir + "/status_check"
check_dir_exists(status_checkdir)  # create a dir for status check
SoftBamDir = createsimilink(args.path, args.outputdir, Bamid_dict)
cov_dict = []

if args.coverage.endswith("coverage.txt"):
    cov_dict = getcoverage(args.coverage)  # coverage is stored under whole bamname#
    # print(cov_dict)

else:
    # find coverage for each sample in teh path of the bamfile folder using gostats

    cov_dict = findcoverage(args.coverage, SoftBamDir, args.outputdir)

print("Coverage dict", cov_dict)
# Run MELT preprocess

try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        count = 0
        for line in f:
            line = line.rstrip()
    ...
    if count == 1:
        ...
        run_MELT_SPLIT_preprocess(args.outputdir, args.meltpath, args.ref_genome, cov_dict, SoftBamDir, args.memmory,
                                  args.cpu)
except:
    print(
        "the status file is not found. It should be created with the creation of the symlink. However proceeding to MELT-processing...")

    # run_MELT_SPLIT_preprocess(args.outputdir, args.meltpath, args.ref_genome, cov_dict, SoftBamDir, args.memmory, args.cpu)
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            run_MELT_SPLIT_indivanal(args.meltpath, args.ref_genome, cov_dict, SoftBamDir, args.memmory, args.cpu,
                                     args.outputdir, args.hg_ver)
except:
    print(
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            run_MELT_split_groupanal(args.meltpath, args.ref_genome, args.memmory, args.cpu, args.outputdir,
                                     args.hg_ver)
except:
    print(
        "the status file is not found or group analysis could not be performed,\n  ")

    # run_MELT_split_groupanal(args.meltpath, args.ref_genome, args.memmory, args.cpu, args.outputdir, args.hg_ver)
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            run_MELT_split_genotype(args.meltpath, args.ref_genome, args.memmory, args.cpu, args.outputdir, args.hg_ver,
                                    cov_dict, SoftBamDir)
except:
    print(
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            run_MELT_split_makeVCF(args.meltpath, args.ref_genome, args.memmory, args.cpu, args.outputdir, args.hg_ver)
except:
    ...
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            try:
                ...
                Run_transurveyor(transpath, SoftBamDir, args.outputdir, args.ref_genome, cov_dict, args.cpu, bwapath,
                             samtoolspath)
                transurveyor_filter(cov_dict, transpath, args.outputdir, args.cpu)
            except:
                print("I am here in step 6.1")
                fix_bampath = Fixmate_Transur(args.table, SoftBamDir, args.picardtool, args.cpu, args.outputdir, samtoolspath)
                fix_bampath = args.outputdir + "/FixedmateBAMs"
                Run_transurveyor(transpath, fix_bampath, args.outputdir, args.ref_genome, cov_dict, args.cpu, bwapath,
                             samtoolspath)
                transurveyor_filter(cov_dict, transpath, args.outputdir, args.cpu)
...
'''
#   Todo: make bed format tables from VCF output- done
#   Make bed format table from Transurveyor output,combine and print single output or print single and concatenate files- done
#   Run Merge SV.script to combine all Trans genotypes- done
#   R
#
'''
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            parse_Melt_VCF_to_bed(args.outputdir, "M")
except:
    print(
...
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            #RMdata_dict = load_Repeatmasker(args.repmask)# loaded repeatmasker to annotate the repeats that are originated from TEs
            #print("loaded RM data")
            parse_Transurveyor_out(args.outputdir, cov_dict)
            #
except:
    print(
    ...
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            mergedsortedfile = concat_files(
                args.outputdir)  # files get bigger all the time because concatenation so need to remove if repeated more than once
            print(mergedsortedfile)
            mergedfilecopy= args.outputdir + "/mergedfileforAnnotSV.bed"
            shutil.copy(mergedsortedfile, args.outputdir + "/mergedfileforAnnotSV.bed" )
...
# Todo: Merge the lines first then do the Repeatmasker check-done

# May be I can make a class is that right- not a good idea as class is for performing multiple functions
#



    # concatenate the files to single one and sort the data--done
# How to do the merging,  modify the script separately and call in the script--done
# Todo: For pedigree studies need to employ the script that follows the segregation pattern and choose (can do it- but not tested)
# Todo: if too slowwill have to split candidates chromosome wise and process that may be will do that in the next stage
# going to run AnnotSV # parse AnnotSV
try:
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        count = 0
        for line in f:
        ...
        if count == 1:
            ...
            try:
                ...
                mergedsortedfile
                annotfullfile,annotallfile = Run_annotSV(args.ASVpath, mergedsortedfile, args.outputdir,args.hg_ver,50)
                fulinputfileforcord_indv, fulinputfileforonlycord = parse_AnnotSV(annotfullfile, args.totaln,
                                                                                  args.scoren,
                                                                                  args.segind,
                                                                                  args.nonsegind, args.outputdir,args.hg_ver)
                copyinputtable1 = args.outputdir + "/fullinputtable_ind_with_brkpt.bed"
                copyinputtable2 = args.outputdir + "/fullinputtable_only_brkpt.bed"
                ...
                mergedsortedfile = args.outputdir + "/mergedfileforAnnotSV.bed"
                annotfullfile,annotallfile = Run_annotSV(args.ASVpath, mergedsortedfile, args.outputdir,args.hg_ver,50)
                fulinputfileforcord_indv, fulinputfileforonlycord = parse_AnnotSV(annotfullfile, args.totaln,
                                                                                  args.scoren,
                                                                                  args.segind,
...
'''
 # Sort on the segregation pattern
 # AnnotSV scores
 # Run ParseannotSV Script to choose candidates either with segregation pattern or with scores
# print the candidates with whole line in one file and print the candidates in the input format suited for my validation scripts
  '''
    ...
    with open(args.outputdir + "/status_check" + "/status_check_file.txt", 'r') as f:
        ...
        if count == 1:
            ...
            try:
                ...
                filefor2ndAnnotSV = call_TSD_to_identifybrkpt(fulinputfileforonlycord, cov_dict, SoftBamDir, args.muscle, args.TElib,
                                                                args.outputdir, args.cpu, args.blastpro,args.totaln)
                # annotfulltsd, annotalltsdfile = Run_annotSV(args.ASVpath, filefor2ndAnnotSV, args.outputdir,
                #                                             args.hg_ver, "1")
            except:
                print("I am here in step11b")
                fulinputfileforonlycord = args.outputdir + "/fullinputtable_only_brkpt.bed"
        ...
        if count == 1:
            ...
            try:
                ...
                if (annotfulltsd and annotalltsdfile):
                    with open(args.outputdir + "/status_check" + "/status_check_file.txt", "a+") as f_obj:
                        f_obj.write("AnnotSV completed on TSDcandidates" + "\n")

            except:
                print("I am here in step12b")
                filefor2ndAnnotSV = args.outputdir + "/filetorunAnSV_2.bed"
            ...
            try:
                ...
                r = subprocess.run(
                    'python3 {} -t {} -n {} -j 11 -s {} -i {} -a {} -cinfo {} -cval {}'.format(
                        direname + "/parseAnnSVTable_forTSDout_v1.1.py", annotalltsdfile, args.totaln, args.scoren,
                        args.segind,
                        args.nonsegind, args.col_info, args.col_value), shell=True)
                # os.system(cmd)
                ...
                annotalltsdfile = args.outputdir + "/AnnotSVoutall/filetorunAnSV_2.annotated.tsv"
...
# r = subprocess.run(
#                     'python3 {} -t {} -n {} -j 11 -s {} -i {} -a {} -cinfo {} -cval {}'.format(
#                         direname + "/parseAnnSVTable_forTSDout_v1.1.py", annotalltsdfile, args.totaln, args.scoren, args.segind,
#                         args.nonsegind,  args.col_info, args.col_value), shell=True)

# in-silico validation
"""
Two types of validation, possible, one step is to do based on discordant reads then to do using split reads
Now my plan is to do only split reads now may be later if needed will incorporate discordant reads

"""
# mergedsortedfile = args.outputdir + "/mergedfileforAnnotSV.bed"
# annotfullfile,annotallfile = Run_annotSV(args.ASVpath, mergedsortedfile, args.outputdir,args.hg_ver)
# annotfullfile = "/ybod2/jainy/Project3/SGDP_outputdir/AnnotSVout/mergedfileforAnnotSV.annotated.tsv"
# fulinputfileforcord_indv,fulinputfileforonlycord = parse_AnnotSV(annotfullfile, args.totaln, args.scoren,
#                                                    args.segind,
#                                                    args.nonsegind, args.outputdir)
# copyinputtable1 = args.outputdir + "/fullinputtable_ind_with_brkpt.bed"
# copyinputtable2 = args.outputdir + "/fullinputtable_only_brkpt.bed"
# shutil.copy(fulinputfileforcord_indv, copyinputtable1)
# shutil.copy(fulinputfileforonlycord, copyinputtable2)
# fulinputfileforonlycord = args.outputdir + "/fullinputtable_only_brkpt_all.bed"
# fulinputfileforonlycord = args.outputdir + "/fullinputtable_only_brkpt_sample.bed"
#
# filefor2ndAnnotSV = call_TSD_to_identifybrkpt(fulinputfileforonlycord, cov_dict, SoftBamDir, args.muscle, args.TElib,
#                                           args.outputdir, args.cpu, args.blastpro,args.totaln)
# print(filefor2ndAnnotSV)
#
# annotfulltsd, annotalltsdfile= Run_annotSV(args.ASVpath,filefor2ndAnnotSV,args.outputdir,args.hg_ver,"1")




'''
# once I have the candidates, I have to go to each individual verify it
# Filter out all denovo insertions that are thought to be neutral based on 1000GP SGDP# or create a db filter out those ones
# extract reads and its mates
# extract the mates and split reads and do blast against mate to see if there is any homology with the respective TE
# count the number of discordant reads # already available with Transurveyor
# Now my plan is to run my perl script to do these steps, later I can incorporate that in python
# should I do in all individuals or only in individuals predicted == should do only individuals predicted to save time
# do only for those that are segregated in the way
# extract mates and identify number of discordant with significant hit to TEs # script 1 (Julie and can perform assembly for >2 Discordant with mates)
# if the number is more than 2 go for Assemble TE and identify TSD, and strand of insertion and coordinate of the TSD and refine breakpoint
# Automatically do IGV screen shot script too for further validation
'''
# to incorporate TypeTE for Alu to generate VCFs
# SVA and lINE-1 is on the way


#Todo:
# also need to incorporate MELT deletion in running the pipeline


# Output format:
#
# unique_id Chr1 TSD_start TSD_end strand TE5p_seq TE3q_seq Todo:No.of.discordantreadswithmate_mappedto_otherTEs gtIndi1 gtind2 gtind3 gtind4
